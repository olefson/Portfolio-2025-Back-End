...
const DiaryRepository_1 = require("../repositories/DiaryRepository");
...
class ChatService {
    constructor(prisma) {
        this.diaryRepository = new DiaryRepository_1.DiaryRepository(prisma);
        this.jobRepository = new JobRepository_1.JobRepository(prisma);
        this.educationRepository = new EducationRepository_1.EducationRepository(prisma);
        this.projectRepository = new ProjectRepository_1.ProjectRepository(prisma);
        this.toolRepository = new ToolRepository_1.ToolRepository(prisma);
    }
    /**
     ...
     */
    async chat(userQuery, conversationHistory = []) {
        try {
...
Return 1-3 most relevant tags. If none are relevant, return an empty array.`;
     ...
     */
    async buildContext(userQuery, relevantTags) {
        ...
        const isInformationalEntry = (entry) => {
                ...
                try {
                    ...
                    if (tavilyResponse.ok) {
                        ...
                        if (data.results && data.results.length > 0) {
                            ...
                            data.results.slice(0, 3).forEach((searchResult, index) => {
    ...
    buildSystemPrompt(context) {
        ...
        const jobsText = context.jobs.length > 0
            ? context.jobs.map(job => `- ${job.title} at ${job.company} (${job.location}) - ${job.type}\n` +
                `  Period: ${this.formatDate(job.startDate)} - ${job.endDate ? this.formatDate(job.endDate) : 'Present'}\n` +
                `  Description: ${job.description}\n` +
                `  Technologies: ${job.technologies.join(', ')}\n` +
                `  Responsibilities: ${job.responsibilities.join('; ')}`).join('\n\n')
            : 'None';
        const educationText = context.education.length > 0
            ? context.education.map(edu => `- ${edu.degree} (${edu.degreeType}) in ${edu.field}\n` +
                ...
                (edu.gpa ? `  GPA: ${edu.gpa}\n` : '') +
                (edu.courses && edu.courses.length > 0 ? `  Courses: ${edu.courses.join(', ')}` : '')).join('\n\n')
            : 'None';
        const projectsText = context.projects.length > 0
            ? context.projects.slice(0, 10).map(project => `- ${project.title || project.name}: ${project.description}${project.githubUrl ? ` (${project.githubUrl})` : ''}`).join('\n')
            : 'None';
        const toolsText = context.tools.length > 0
            ? context.tools.slice(0, 20).map(tool => `- ${tool.name}: ${tool.description}`).join('\n')
            : 'None';
        ...
        const informationalText = context.informational.length > 0
            ? context.informational.map(entry => {
                var _a;
                const title = ((_a = entry.title) === null || _a === void 0 ? void 0 : _a.startsWith('&&'))
...
- If you don't have complete information, be honest about what you know and don't know`;
     ...
     */
    getDiaryExcerpt(content, maxLength = 150) {
